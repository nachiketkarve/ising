...
int main(int argc, char *argv[])
{
    ...
    if (argc != 3)
    {
        throw std::invalid_argument("usage: ./build/bin/cyclicPerturbationKrylov.exe <maxTime> <lambda>");
    }
    ...
    for(int average = 0; average < averages; average++)
    {
        ...
        {
            evolveKrylov(state, N, hbar, J, hz, hx, lambda, mu, dt * iter, dt, krylovDim);
        }
